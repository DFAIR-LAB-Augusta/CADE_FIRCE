...
def main() -> None:
    ...
    saved_unnormalized_path = os.path.join(
        UNNORMALIZED_SAVE_FOLDER, f'{args.name}_unnormalized.npz'
    )
...
def parse_args():
    ...
    p.add_argument(
        '--sampling-ratio', type=float, default=True, help='The ratio of downsampling.'
    )
    args = p.parse_args()
...
def split_data(args, saved_unnormalized_path):
    if os.path.exists(saved_unnormalized_path):
        ...
        for (
            mal_day,
            mal_category,
        ) in new_file_types_dict.items():  # only one (key, value) in this dict.
...
def normalize(X_train, X_test, y_train, y_test, ratio, save_path) -> None:
    ...
    training_ports_transform = transform_ports_to_categorical(
        training_ports, high_freq_port_list, medium_freq_port_list, low_freq_port_list
    )
    port_encoder = OneHotEncoder(sparse=False, handle_unknown='ignore')
    training_ports_transform = np.array(training_ports_transform).reshape(
        len(training_ports_transform), 1
    )
    ...
    )  # ignore will encode unseen protocol as all 0
    training_protocols = X_train[:, 1].reshape(
        len(X_train), 1
    )  # when keeping Dst port, protocol is the 2nd feature
    ...
    X_train_scale = scaler.fit_transform(
        X_train[:, 2:]
    )  # MinMax the rest of the features
    ...
    testing_ports_transform = transform_ports_to_categorical(
        testing_ports, high_freq_port_list, medium_freq_port_list, low_freq_port_list
    )
    testing_ports_transform = np.array(testing_ports_transform).reshape(
        len(testing_ports_transform), 1
    )
    ...
    X_new_normalize = np.concatenate(
        (testing_ports_encoded, test_protocols_encoded), axis=1
    )
...
def downsampling(X_train, y_train, ratio, phase):
    ...
    for idx, family in enumerate(np.unique(y_train)):
        ...
        filter_idx = np.random.choice(
            family_idx, size=int(ratio * family_size), replace=False
        )
        ...
        if idx == 0:
            ...
            X_train_sampling = np.concatenate(
                (X_train_sampling, X_train_family), axis=0
            )
            y_train_sampling = np.concatenate(
                (y_train_sampling, y_train_family), axis=0
            )
    return X_train_sampling, y_train_sampling


def transform_ports_to_categorical(
    ports, high_freq_port_list, medium_freq_port_list, low_freq_port_list
):
    ports_transform = []
    for port in ports:
