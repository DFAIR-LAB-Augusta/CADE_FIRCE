...
class MLPClassifier:
    ...
    def build(self):
        ...
        for i in range(n_stacks - 1):
            x = Dense(self.dims[i + 1], activation=self.act, name='clf_%d' % i)(x)
            if self.dropout > 0:
                x = Dropout(self.dropout, seed=42)(x)
        x = Dense(self.dims[-1], activation='softmax', name='clf_%d' % (n_stacks - 1))(
            x
        )
        output_tensor = x
    ...
    def train(
        ...
        if train_val_split:  # used for normal training process
            ...
            if retrain:
                ...
                model.compile(
                    loss=loss, optimizer=pretrain_optimizer, metrics=['accuracy']
                )
                ...
                plt.xlabel('epoch')
                plt.savefig(
                    self.model_save_name + f'_lr{lr}_epoch{epochs}_loss.png', dpi=200
                )
                plt.clf()
