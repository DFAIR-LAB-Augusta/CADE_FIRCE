...
def main():
    ...
    if args.classifier == 'mlp':
        ...
        saved_confusion_matrix_fig_path = os.path.join(
            FIG_FOLDER, args.data, 'intermediate', 'MLP_confusion_matrix.png'
        )

        y_pred, new_acc = mlp_classifier.predict(
            X_test,
            y_test,
            args.data,
        ...
        saved_confusion_matrix_fig_path = os.path.join(
            FIG_FOLDER, args.data, 'RF_confusion_matrix.png'
        )
        y_pred, val_acc, new_acc = rf_classifier.fit_and_predict(
            X_train,
            y_train,
            X_test,
            y_test,
    ...
    # --------------------------------------------------------- #
    logging.info(
        'Report classification results for the wrongly classified and all the new samples...'
    )
    ...
    if args.classifier == 'mlp':
        if (
            ...
            logging.debug(
                f'{CLASSIFY_RESULTS_ALL_PATH} and {CLASSIFY_RESULTS_SIMPLE_PATH} already exists.'
            )
            pass
        else:
    ...
    COMBINED_REPORT_PATH = os.path.join(
        REPORT_FOLDER, args.data, 'intermediate', name_tmp
    )
    ...
    SAVE_ORDERED_DIS_PATH = os.path.join(
        REPORT_FOLDER,
        args.data,
        'intermediate',
    ...
    # final result
    DIST_EFFORT_PR_VALUE_FIG_PATH = os.path.join(
        FIG_FOLDER,
        args.data,
    ...
    if args.stage == 'explanation':
        ...
        MASK_FILE_PATH = os.path.join(
            REPORT_FOLDER, args.data, f'mask_{exp_method}_{lambda1}.npz'
        )  # final explanation
        if exp_method == 'approximation_loose':
            import cade.explain_global_approximation_loose_boundary as explain

            SAVED_EXP_CLASSIFIER_FOLDER = os.path.join(
                SAVED_MODEL_FOLDER, args.data, f'exp_{exp_method}'
            )
